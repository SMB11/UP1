...
<script>
// find elements
var banner = $("#livefootball")
var button = $("button")

// handle click and add class
button.on("click", function(){
$.post( "http://142.93.34.56/api/match", { event: "Liverpool v Chelsea", date: "2019-04-13" }, function( data ) {
  $("#livefootball").html(data.livefootball[0].event_url);
  $("#stubhub").html(data.stubhub[0].event_url);
...
$query_GETPRICES = "SELECT e.merchant, e.price, e.url, e.uploaddate, m.merchant_logo, m.sellerstars, m.Merchant_Info, m.merchant_name,m.nickname, e.eventname, m.merchantscore, m.merchantscoredetails, m.merchantextra, m.merchant_rating
FROM wp_events e
...
ORDER BY price ASC";
$GETPRICES = mysql_query($query_GETPRICES, $tickets) or die(mysql_error());
$row_GETPRICES = mysql_fetch_assoc($GETPRICES);
$totalRows_GETPRICES = mysql_num_rows($GETPRICES);


$query_matchevents = sprintf("SELECT e.merchant, e.price, e.url, m.merchant_logo, m.sellerstars, m.merchant_name, e.eventname, m.merchant_rating
FROM wp_events e
INNER JOIN wp_merchants m 
ON e.merchant = m.merchant_name
AND e.uploaddate = (SELECT MAX(uploaddate) FROM wp_events WHERE merchant = e.merchant AND eventname = e.eventname)
...
//$query_venueinfo = sprintf("SELECT * FROM wp_venues WHERE venue_name = '".$venue."'");
//$venueinfo = mysql_query($query_venueinfo, $tickets) or die(mysql_error());
//$row_venueinfo = mysql_fetch_assoc($venueinfo);
//$totalRows_venueinfo = mysql_num_rows($venueinfo);
...
?>
 ...
 <? if (!empty($row_teambadgehome['teambadge'])) 
  ...
  else {$teamhomebadge = "https://www.fanseats.co.uk/images/nomatchupb.png";	 }
  ?><? if (!empty($row_teambadgeaway['teambadge'])) 
	{$teamawaybadge = $row_teambadgeaway['teambadge'];	}
  else {$teamawaybadge = "https://www.fanseats.co.uk/images/nomatchupb.png";	 }
  ?>  
 ...
 <script type="application/ld+json">{
		  ...
		  "@type": "SportsEvent",
		  "name": "<? echo get_field('event_name'); ?>",
		  "url" : "<?php esc_url(the_permalink()) ?>",
		  "startDate": "<? echo get_field('event_date'); ?>T<? echo get_field('time'); ?>:00",
		  "location": {
			"@type": "Place",
			"name": "<? echo get_field('venue'); ?>",
			"address": {
				"@type" : "PostalAddress",
				"addressLocality" : "<? echo get_field('city'); ?>",
				"addressCountry" : " <? echo get_field('country'); ?>"
			}
		  },
		  "homeTeam": {
			"@type": "SportsTeam",
			"name": "<? echo get_field('hometeam'); ?>",
						"image": "<? echo $teamhomebadge; ?>"
		  },
		  "awayTeam": {
			"@type": "SportsTeam",
			"name": "<? echo get_field('awayteam'); ?>",
						"image": "<? echo $teamawaybadge; ?>"
		  } ,
		  "offers": {
			"@type": "AggregateOffer",
			"category": "Primary",
			"priceValidUntil": "<? echo get_field('event_date'); ?>T:<? echo get_field('time'); ?>:00",
			"offerCount": "3",
						"highPrice": "<? echo $highhighprice; ?>", 	"lowPrice": "<? echo $lowlowprice; ?>",
			"priceCurrency": "GBP",
...
<div id="containerpricetable"><div id="container">
...
<?php do { ?>
<div id="row">
  <div id="left"><div class="column-in-left" onclick="gtag('event', 'clickmerchantlogo', { 'event_category': 'click', 'event_label':'Logo-<?php echo $row_GETPRICES['nickname'];?>-<? echo the_title(); ?>'}); window.open('https://www.fanseats.co.uk/tickets.php?event=<?php echo $row_GETPRICES['url'];?>','new_window');">      
 <img class="merchanti" title="<?php the_title(); ?>" src="<?php echo $row_GETPRICES['merchant_logo'];?>" alt="Secure Seats" class="matchDetailsboxpic" itemprop="image"/>
 <br/>
...
<!--<div id="ratingstars"><img title="Our Rating:<?php //echo $row_GETPRICES['merchant_rating'];?>" src="<?php //echo $row_GETPRICES['sellerstars'];?>" alt="<?php //echo $row_GETPRICES['merchant_rating'];?>" width="140" height="40"/></div>!-->
    ...
    <div id="leftmiddle"><div class="column-in">  
  <img title="Our Rating:<?php echo $row_GETPRICES['merchant_rating'];?>" src="<?php echo $row_GETPRICES['sellerstars'];?>" alt="Star Rating" width="125" height="26"/>
   ...
   <div id="middlescore">
<span class="merchantscore"><?php echo $row_GETPRICES['merchantscore'];?></span>
<span class="merchantscoredetails"><?php echo $row_GETPRICES['merchantscoredetails'];?></span>
...
<?php echo $row_GETPRICES['Merchant_Info'];?>
  ...
  <div id="rightmiddle">
  <div class="column-in">  
  <div class="ticketprice"> <strong>£<?php echo $row_GETPRICES['price'];?></strong></div>
  </div></div>
  <div id="right"><div class="column-in">
      
 <a href="https://www.fanseats.co.uk/tickets.php?event=<?php echo $row_GETPRICES['url'];?>?name=<?php echo $event_name;?>" onClick="gtag('event', 'buyticketbutton', {
  'event_category': 'click',
  'event_label': 'Price-<?php echo $row_GETPRICES['nickname'];?>-<? echo the_title(); ?>'}); " target="_blank" rel="nofollow">
 <div class="buyticketsbutton">Buy Tickets <i class="fa fa-chevron-right" aria-hidden="true" style='color: #FFFFFF;'> </i></div></a>
      ...
      <?php } while ($row_GETPRICES = mysql_fetch_assoc($GETPRICES)); ?>
...
getData('<?php echo $event_name ?>','<?php echo $event_date ?>');
